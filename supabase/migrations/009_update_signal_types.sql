...
-- Drop existing constraint on etf_economic_signals table if it exists
ALTER TABLE etf_economic_signals
DROP CONSTRAINT IF EXISTS etf_economic_signals_signal_check;

-- Add updated constraint to etf_economic_signals
ALTER TABLE etf_economic_signals
ADD CONSTRAINT etf_economic_signals_signal_check
CHECK (signal IN ('BUY', 'SELL', 'HOLD', 'STRONG BUY', 'STRONG SELL'));
